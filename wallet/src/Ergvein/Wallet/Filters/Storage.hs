...
clearFilters cur = do
  e <- getFiltersStorage
  case cur of
    Bitcoin -> liftIO $ readWriteTransaction e BTC.cleanBtcDbs
    _ -> pure ()
...
clearFiltersRange cur i0 i1 = do
  e <- getFiltersStorage
  case cur of
    Bitcoin -> liftIO $ readWriteTransaction e $ BTC.clearFiltersRange i0 i1
    _ -> pure ()
...
getFiltersHeight cur = do
  e <- getFiltersStorage
  case cur of
    Bitcoin -> BTC.readFiltersHeight e
    Ergo -> pure $ filterStartingHeight ERGO -- TODO: here

watchFiltersHeight :: (MonadIO m, HasFiltersStorage t m, MonadHold t m, Reflex t, MonadFix m) => Currency -> m (Dynamic t (Maybe BlockHeight))
watchFiltersHeight cur = do
  md <- externalRefDynamic =<< getFiltersHeightRef
  holdUniqDyn $ M.lookup cur <$> md
...
insertFilter cur h bh f = do
  e <- getFiltersStorage
  case cur of
    Bitcoin -> BTC.insertFilter h bh f e
    Ergo -> pure () -- TODO: here
...
insertMultipleFilters cur fs = do
  e <- getFiltersStorage
  case cur of
    Bitcoin -> BTC.insertMultipleFilters fs e
    Ergo -> pure () -- TODO: here
  writeFiltersHeight cur =<< getFiltersHeight cur
...
getFilter c bh = do
  e <- getFiltersStorage
  case c of
    Bitcoin -> fmap (fmap AddrFilterBtc) $ BTC.readFilter bh e
...
foldFilters c f a0 = do
  e <- getFiltersStorage
  case c of
    Bitcoin -> BTC.foldFilters (\i n k -> f i n k . AddrFilterBtc) a0 e
...
scanFilters c i0 f a0 = case c of
  Bitcoin -> scanBtcFilters i0 (\i n k -> f i n k . AddrFilterBtc) a0
  Ergo -> pure a0 -- ^ TODO: add ergo here
